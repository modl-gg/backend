    ...
    public List<Document> aggregateLogActivityBySource(Server server, Date startDate) {
        ...
        Aggregation aggregation = Aggregation.newAggregation(
                ...
                Aggregation.group(AuditLogFields.SOURCE)
                        ...
                        .sum(ConditionalOperators.when(MongoQueries.where(AuditLogFields.DESCRIPTION).regex(Pattern.compile("ticket", Pattern.CASE_INSENSITIVE))).then(1).otherwise(0)).as(ALIAS_TICKET_ACTIONS)
    ...
    public long countEvidenceUploads(Server server, String username, Date startDate) {
        ...
        Query query = Query.query(
                baseCriteria.orOperator(
                        ...
                        MongoQueries.where(AuditLogFields.LEVEL).is(LEVEL_INFO).and(AuditLogFields.DESCRIPTION).regex(Pattern.compile("uploaded|attachment", Pattern.CASE_INSENSITIVE))
                )
        );
