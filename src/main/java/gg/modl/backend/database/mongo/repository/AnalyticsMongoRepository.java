...
public class AnalyticsMongoRepository {
    ...
    public Document aggregatePunishmentAnalytics(Server server, Date startDate, String analyticsTimeZone) {
        ...
        final Document byTypeFacet = new Document("$group", new Document("_id", "$" + PlayerFields.PUNISHMENT_TYPE_ORDINAL).append(ALIAS_COUNT, new Document("$sum", 1)));
        final Document sortByCountDesc = new Document("$sort", new Document(ALIAS_COUNT, -1));
        final Document byStaffFacet = new Document("$group", new Document("_id", "$" + PlayerFields.PUNISHMENT_ISSUER_NAME).append(ALIAS_COUNT, new Document("$sum", 1)));
